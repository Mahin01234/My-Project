...
def predict_insurance_charge(age, sex, bmi, children, smoker, region):
    ...
    description = f"Input Parameters: Age={age}, Sex={sex}, BMI={bmi}, Children={children}, Smoker={smoker}, Region={region}"

    return f"Predicted Insurance Charge: ${prediction:,.2f}", description

# Gradio Interface
interface = gr.Interface(
    fn=predict_insurance_charge,
    inputs=[
        gr.Number(label="Age (18-64)", minimum=18, maximum=64, step=1, value=30),
        gr.Radio(['female', 'male'], label="Sex", value='male'),
        gr.Number(label="BMI", minimum=16.0, maximum=53.0, step=0.1, value=28.5),
        gr.Slider(0, 5, step=1, label="Number of Children", value=2),
        gr.Radio(['no', 'yes'], label="Smoker", value='no'),
        gr.Dropdown(['northeast', 'northwest', 'southeast', 'southwest'], label="Region", value='southeast')
    ],
    outputs=[
        gr.Textbox(label="Predicted Insurance Charge"),
        gr.Textbox(label="Input Details")
    ],
    title="Insurance Charge Prediction App",
    description="Enter patient details to get an estimated insurance charge."
)
